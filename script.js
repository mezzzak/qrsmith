document.addEventListener('DOMContentLoaded', function () {
  console.log("QRSMITH logic initialized...");
  ...
  (function () {
    ...
    function setButtonsEnabled(enabled) {
      ...
      if (btnSvg) btnSvg.disabled = !enabled;
    ...
    function renderPreview() {
      ...
      QRCode.toCanvas(canvas, text, currentOptions(320), function (err) {
        if (err) {
          previewCaption.textContent = 'Could not encode this content.';
          setButtonsEnabled(false);
          return;
        }
        hasContent = true;
    ...
    if (ecLevelSelect) ecLevelSelect.addEventListener('change', debouncedRender);
    if (btnPng) btnPng.addEventListener('click', exportPng);
    if (btnSvg) btnSvg.addEventListener('click', exportSvg);
    if (btnPdf) btnPdf.addEventListener('click', exportPdf);

    // Initial state
    if (fgColorInput) fgColorText.textContent = fgColorInput.value.toUpperCase();
    if (bgColorInput) bgColorText.textContent = bgColorInput.value.toUpperCase();
    setButtonsEnabled(false);
